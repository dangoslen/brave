 ...
 * state, and defer associating it with a span ID (via {@link ExtraFactory#decorate(TraceContext)}.
 *
...
public abstract class Extra<E extends Extra<E, F>, F extends ExtraFactory<E, F>> {
  ...
  final AtomicReference<Object> internal = new AtomicReference<>();
  long traceId;
  ...
  protected Extra(F factory) {
    ...
    this.internal.set(factory.initialState);
  }
   ...
   *
   * @return Updated state that will be set to this instance later via {@link
   * ExtraFactory#compareAndSetState(Extra, Object, Object)}
   */
  protected abstract Object mergeStateKeepingOursOnConflict(E that);
